...
@modified: Di 5. Sep 12:09:16 CEST 2017
...
if __name__=="__main__":
  ...
  ref_nnl_data_open=open(REF_NNL_FILENAME,'r')
  cur_nnl_data_open=open(CUR_NNL_FILENAME,'r')
  ...
  #####################################
  # Reading Reference NNL Data for cut off radius
  for line in ref_nnl_data_open:
    data=line.strip().split()
    if data[0] == "#":
       ref_nnl_rcut=data[5]

  #####################################
  # Reading Current NNL Data for cut off radius
  for line in cur_nnl_data_open:
    data=line.strip().split()
    if data[0] == "#":
       cur_nnl_rcut=data[5]
  ...
  file1.write("## computed nnlchkpt using cur_chkpt=%s \n"%(CUR_CHKPT_FILENAME))
  file1.write("## computed nnlchkpt using ref_nnl=%s \n"%(REF_NNL_FILENAME))
  file1.write("## computed nnlchkpt using cur_nnl=%s \n"%(CUR_NNL_FILENAME))
  file1.write("## ref_nnl_cut_off_radius= %s \t cur_nnl_cut_off_radius= %s \n"%(ref_nnl_rcut,cur_nnl_rcut))
